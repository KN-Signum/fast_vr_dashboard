...
```json
{
  ...
  "notes": "Optional notes before the session"
}
```
...
- `PUT /api/sessions/{session_id}/post-session-notes`
- `GET /api/sessions/{session_id}/download/summary`
- `GET /api/sessions/{session_id}/download/raw`
- `POST /api/sessions/{session_id}/upload/raw`

Ending or exporting the active session first drains pending writes. The summary
download is a Polish A4 PDF generated by the backend. It includes session and
patient metadata, pre/post-session notes, stream counts, the latest generated
and patient-reported bird counts, eye-tracking analysis, and observed events.
Raw download is a ZIP containing `session.json` and all NDJSON streams.

Post-session notes can be added or replaced after the session has ended:

```json
{"notes": "Optional observations after the session"}
```
